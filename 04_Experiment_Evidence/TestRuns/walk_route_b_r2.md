...
- Adaptive tuning: Off
- Session ID: 20260602_225757_528
- Raw log file: TestLogs/20260602_225757_528_user_position_history.json
...
- Duration: approx 46 seconds logged
...
- Matching pulled log is `20260602_225757_528_user_position_history.json`.
- Pulled log contains 11 samples.
- App trial label is `WALK`.
- Adaptive tuning was off.
- Sample 0 is the manual `ANCHOR` / `MAP_TAP` at approximately `(0.5716, 1.9017)`.
- All remaining samples are `FUSION` tracking-mode samples, but none had an accepted BLE fix.
- Samples 1, 2, 5, 6, 7, 8, 9, and 10 are `DEAD_RECKONING`.
- Samples 3 and 4 show `LESS_THAN_3_BEACONS`.
- BLE fix ratio is `0.0000`, so 0 of 11 samples had accepted BLE fixes.
- This run should not be used as walking validation evidence.

## Problems

- No accepted BLE fixes were recorded.
- The route contains a valid manual anchor, but the subsequent path is dead reckoning or beacon-loss output only.
- The path reached map boundaries near `(0.00, 2.50)` and later near `(2.50, 2.50)`, which indicates boundary clamping during dead reckoning.
- Because there were no accepted BLE fixes, this repeat does not validate BLE-assisted walking behavior.

## Path Behavior Summary

- This run is rejected for Route B validation because it contains no accepted BLE fixes after the manual anchor. It is retained only as a failed/diagnostic walking attempt showing dead-reckoning behavior under missing BLE correction.
